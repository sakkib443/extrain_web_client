...
export default function UserPurchasesPage() {
    ...
    const filteredOrders = orders.filter(order => {
        ...
        const matchesSearch = !searchTerm ||
            orderNum.includes(searchLow) ||
            order.items?.some(item => (item.title || "").toLowerCase().includes(searchLow));
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className={`${cardClass} p-5 relative group overflow-hidden`}>
                    <div className="relative z-10 flex items-start justify-between">
                        ...
                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-emerald-500 to-rose-600 flex items-center justify-center text-white shadow-md group-hover:scale-110 transition-transform duration-300">
                ...
                <div className={`overflow-hidden ${cardClass}`}>
                    ...
                    <div className={`hidden md:grid grid-cols-12 gap-4 px-6 py-4 text-[10px] font-bold uppercase tracking-widest ${isDark
                        ...
                        <div className="col-span-3 flex items-center gap-2">
                            <span className="w-1.5 h-1.5 rounded-full bg-[#FD9A00]"></span>
                            Order ID
                        </div>
                    ...
                    <div className="divide-y divide-slate-100 dark:divide-white/5">
                        {filteredOrders.map((order, idx) => (
                            <div key={order._id} className={`${expandedOrder === order._id ? isDark ? 'bg-slate-800/20' : 'bg-[#FD9A00]/5' : ''}`}>
                                {/* Main Row */}
                                <div
                                ...
                                >
                                    ...
                                    <div className="hidden md:flex col-span-1 justify-center items-center">
                                        <button className={`p-2.5 rounded-xl transition-all ${expandedOrder === order._id
                                            ? isDark ? 'bg-[#FD9A00]/10 text-[#FD9A00]' : 'bg-[#FD9A00]/10 text-[#FD9A00]'
                                        ...
                                        <div className={`rounded-xl border overflow-hidden ${isDark ? 'border-white/5' : 'border-slate-200'}`}>
                                            ...
                                            <div className={`flex flex-wrap items-center justify-between gap-3 px-4 py-3 ${isDark ? 'bg-slate-800/50' : 'bg-slate-50'}`}>
                                                ...
                                                <button className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-bold transition-colors ${isDark
                                                    ...
                                                    : 'bg-white text-slate-500 hover:text-[#FD9A00] border border-slate-200'
                                                    }`}>
            ...
            <div className={`p-5 rounded-2xl border flex items-center justify-between gap-4 ${isDark
                ...
                <div className="flex items-center gap-3">
                    <FiCreditCard size={18} className="text-[#FD9A00] shrink-0" />
                    <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                        Need help with a payment? <Link href="/dashboard/user/support" className="text-[#FD9A00] font-bold hover:underline">Contact Support</Link>
                    </p>
                </div>
            </div>
        </div>
