...
import {
    FiArrowLeft, FiCreditCard, FiPlus, FiTrash2, FiRefreshCw, FiX, FiSave, FiCalendar,
} from 'react-icons/fi';
...
export default function ExpensesPage() {
    ...
    const [form, setForm] = useState({ reason: '', category: 'ads', amount: '', costDate: new Date().toISOString().slice(0, 10) });
    const [saving, setSaving] = useState(false);

    const load = async () => {
        setLoading(true);
        try {
            const [exp, mon] = await Promise.all([ptApi.getExpenses(month), ptApi.getMonths()]);
            setExpenses(exp);
            setMonths(mon);
    ...
    const save = async () => {
        ...
        try {
            ...
            setShowForm(false);
            setForm({ reason: '', category: 'ads', amount: '', costDate: new Date().toISOString().slice(0, 10) });
            load();
        } catch (e) { toast.error(e.message); }
    ...
    const remove = async (id) => {
        ...
        try { await ptApi.deleteExpense(id); toast.success('ডিলিট হয়েছে'); load(); }
        catch (e) { toast.error(e.message); }
    };
    ...
    return (
        <div className="space-y-6">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className={`text-2xl font-bold flex items-center gap-2 ${isDark ? 'text-white' : 'text-slate-800'}`}>
                            <FiCreditCard style={{ color: '#FD9A00' }} /> Expenses
                        </h1>
                        <p className={`text-sm ${muted}`}>মাসিক খরচের হিসাব</p>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    <select className={input} value={month} onChange={(e) => setMonth(e.target.value)}>
                        <option value="">সব মাস</option>
                        {months.map((m) => <option key={m.monthKey} value={m.monthKey}>{monthLabel(m.monthKey)}</option>)}
                    </select>
            ...
            {/* Total */}
            <div className={`rounded-2xl p-5 border ${isDark ? 'bg-slate-800/60 border-slate-700' : 'bg-white border-slate-200'} flex items-center justify-between`}>
                <span className={`font-semibold ${muted}`}>{month ? monthLabel(month) : 'সর্বমোট'} খরচ</span>
                <span className="text-2xl font-bold text-rose-500">{bdt(total)}</span>
            </div>
                    ...
                    <table className="w-full">
                        ...
                        <tbody className={isDark ? 'divide-y divide-slate-700/50' : 'divide-y divide-slate-100'}>
                            {loading ? (
                                ...
                                <tr><td colSpan={6} className={`text-center py-14 ${muted}`}>কোনো খরচ নেই।</td></tr>
                            ) : expenses.map((e, i) => (
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className={`relative w-full max-w-md rounded-2xl shadow-2xl p-6 ${isDark ? 'bg-slate-900 border border-slate-700' : 'bg-white'}`}>
                        <div className="flex items-center justify-between mb-5">
                            <h2 className={`text-lg font-bold ${isDark ? 'text-white' : 'text-slate-800'}`}>Add Expense</h2>
